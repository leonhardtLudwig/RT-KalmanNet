...
class RT_KalmanNet_nn(nn.Module):
    def __init__(self, input_size_fcl, gru_hidden_size, c_init = 1e-3):
        """
        Args:
            ...
            c_init (float): Valore iniziale desiderato per c_t dopo softplus.
        """
        super().__init__()
        ...
        # --- IMPORTANT: init bias in inverse-softplus space so softplus(bias) ~= c_init ---
        c_init = float(c_init)
        c_init = max(c_init, 1e-8)  # strictly positive
        bias_init = torch.log(torch.expm1(torch.tensor(c_init, dtype=self.fcl_out.bias.dtype)))
        with torch.no_grad():
            self.fcl_out.bias.fill_(bias_init.item())
